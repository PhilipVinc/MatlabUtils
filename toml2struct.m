function Result = toml2struct(FileName)
%==========================================================================
...
while ~feof(f)                          % and read until it ends
    ...
    if ( s(1)=='[' ) && (s(end)==']' )
        ...
        [par,val] = strtok(s, '=');
        par = strrep(par, '"', ''); % remove string indicators
        subKeys = strsplit(par, '.');
        val = CleanValue(val);
        %strDelCount = count(val, '"');
        if (val(1)=='"' && val(end) == '"') % isstring
            val = val(2:end-1);
        elseif (val(1)=='[' && val(end) == ']')
            val = strrep(val, ']', '];');
            [val success] = str2num(val);
        else
            [val success] = str2num(val);
        end
        ...
        elseif (length(subKeys) == 2)
            if ~isempty(CurrMainField)
                % But we found section before and have to fill it
                Result.(CurrMainField).(genvarname(subKeys{1})).(genvarname(subKeys{2})) = val;
            else
                % No sections found before. Orphan value
                Result.(genvarname(subKeys{1})).(genvarname(subKeys{2})) = val;
            end
        else
            fprintf('ERROR');
        end
